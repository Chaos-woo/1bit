import 'package:cw2bit/modules/feature_explore/github_1bit/issues/components/github_1bit_issues_list/logic.dart';
...
class Github1bitIssuesLogic extends GetxController {
  ...
  void onReady() async {
    Get.find<Github1bitIssuesListLogic>().on_refreshed_listener = (bool is_refresh_fetch, bool is_fetch_success) async {
      if (is_fetch_success) {
        /// 更新issues列表总数
        state.m_total_issues_count.value = Get.find<Github1bitIssuesListLogic>().state.dataList.length;
      }
    };
