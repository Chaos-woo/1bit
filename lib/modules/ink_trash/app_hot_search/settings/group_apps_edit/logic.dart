...
final class _PathVariables {
  String get group_id => q0_.route.pathVariables(path_k_group_id)!;
...
class AppHotSearchGroupAppsEditLogic extends GetxController {
  ...
  Future<void> refresh_all_apps() async {
    _apps = Set.from(await c0_.data_repository.hot_search.list_apps_by_group_id(group.id!));
  ...
  Future<void> save_apps_order(int old_index, int new_index) async {
    ...
    await c0_.data_repository.hot_search.save_apps_order(new_apps);
  ...
  Future<void> add_or_delete_app(String app_name) async {
    if (is_in_local_database(app_name)) {
      ...
      await c0_.data_repository.hot_search.delete_apps(all_deleted_app_ids);
      _apps = Set.from(await c0_.data_repository.hot_search.list_apps_by_group_id(group.id!));
    } else {
      await c0_.data_repository.hot_search.add_app(app_name, group.id!, -1);
      _apps = Set.from(await c0_.data_repository.hot_search.list_apps_by_group_id(group.id!));
    }
  ...
  Future<void> delete_app(int id) async {
    await c0_.data_repository.hot_search.delete_app(id);
    _apps = Set.from(await c0_.data_repository.hot_search.list_apps_by_group_id(group.id!));
    update([k_groups_dnd_view_id]);
  }
}
